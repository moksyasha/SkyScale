...
class FastFlowNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.interpolate(x, size=(h_up, w_up), mode='bilinear', align_corners=False).contiguous()
        ...
        # flow2[:, 0, :, :] *= float(W) / float(w_up)
        # flow2[:, 1, :, :] *= float(H) / float(h_up)

        # if self.training:
        #     return flow2, flow3, flow4, flow5, flow6
        # else:
            # div_flow * intepolat
        return 20 * F.interpolate(flow2, size=(H, W), mode='bilinear', align_corners=False)